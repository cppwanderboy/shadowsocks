...
static int
server_handshake_reply(EV_P_ ev_io *w, int udp_assc,
  ...
  if (udp_assc) {
    ...
    if (getsockname(server->fd, (struct sockaddr *) &sock_addr, &addr_len) <
        0) {
  ...
  if (atyp == SOCKS5_ATYP_IPV4 || atyp == SOCKS5_ATYP_IPV6) {
    if (dst_port == http_protocol->default_port)
      ...
      hostname_len = tls_protocol->parse_packet(buf->data + 3 + abuf->len,
                                                buf->len - 3 - abuf->len,
                                                &hostname);
    if (hostname_len == -1 && buf->len < SOCKET_BUF_SIZE &&
      ...
      if (acl || verbose) {
        hostname_len =
            hostname_len > MAX_HOSTNAME_LEN ? MAX_HOSTNAME_LEN : hostname_len;
      ...
      if (atyp == SOCKS5_ATYP_DOMAIN
        ...
        switch (((struct sockaddr *) &storage)->sa_family) {
          ...
          case AF_INET6: {
            ...
            if (!inet_ntop(AF_INET6, &(addr_in6->sin6_addr), ip,
                           INET6_ADDRSTRLEN))
              goto not_bypass;
            break;
          }
  ...
  while (1) {
    ...
    if (server->stage == STAGE_STREAM) {
      ...
      if (verbose) {
        ...
        if (getpeername(server->fd, (struct sockaddr *) &peer_addr,
                        &peer_addr_len) == 0) {
          LOGI("connection from %s:%hu", inet_ntoa(peer_addr.sin_addr),
...
create_remote(listen_ctx_t *listener,
  ...
  if (listener->mptcp > 1) {
    ...
    while ((listener->mptcp = mptcp_enabled_values[i]) > 0) {
      int err = setsockopt(remotefd, SOL_TCP, listener->mptcp, &opt,
                           sizeof(opt));
      if (err != -1) {
        break;
      }
  ...
  remote_t *remote = new_remote(remotefd, direct ? MAX_CONNECT_TIMEOUT
                                                 : listener->timeout);
  ...
  if (verbose) {
    ...
    LOGI("remote: %s:%hu", inet_ntoa(sockaddr->sin_addr),
...
main(int argc, char **argv) {
  ...
  for (i = 0; i < remote_num; i++) {
    ...
    struct sockaddr_storage *storage = ss_malloc(
  ...
  if (mode != TCP_ONLY) {
    ...
    char *port =
        remote_addr[0].port == NULL ? remote_port : remote_addr[0].port;
    struct sockaddr_storage *storage = ss_malloc(
        sizeof(struct sockaddr_storage));
    memset(storage, 0, sizeof(struct sockaddr_storage));
    if (get_sockaddr(host, port, storage, 1, ipv6first) == -1) {
  ...
  if (mode != UDP_ONLY) {
    ...
    for (i = 0; i < listen_ctx.remote_num; i++) ss_free(
